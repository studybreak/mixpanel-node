...
var client = function(token) {
    ...
    */
    ...
    /**
        engage(distinct_id, properties, callback)
        ---
        this function sends engage (people analytics) data to mixpanel
        
        distinct_id:string              the user's distinct id
        properties:object               the people properties to set
        callback:function(err:Error)    callback is called when the request is
                                        finished or an error occurs
    */
    metrics.engage = function (distinct_id, properties, callback) {
        if (!properties) { properties = {}; }

        var data = {
            '$set' : properties,
            '$distinct_id': distinct_id,
            '$token': properties.$token || metrics.token
        }
        if (metrics.config.debug) {
            console.log("Sending the following engage data to Mixpanel:");
            console.log(data);
        }
